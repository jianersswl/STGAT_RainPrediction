...
class ClassifyByThresholds:
    ...
    def __call__(self, grid: torch.Tensor) -> torch.Tensor:
        ...
        result  = torch.ones(grid.shape)*-1
        for i, start in enumerate(thresholds[:-1]):
            end = thresholds[i + 1]
            result = torch.where((start <= grid) & (grid < end), i * torch.ones(grid.shape), result)
        # print("result",result)
        # 将张量展平成一维
        result_classified_flat = result.to(torch.int64).view(-1)
        
        # 统计每种分类标签的数量
        # print(result_classified_flat.shape)
        # print(torch.max(result_classified_flat), torch.min(result_classified_flat), (result_classified_flat<0).sum())
        valid_values = result_classified_flat[result_classified_flat >= 0]
        label_counts = torch.bincount(valid_values)
        # print(label_counts)
        return result
