...
class CrossEntropyLoss_Two(ClassificationStat):
    def __init__(self, args, device, num_classes, experiment_name=None):
        ...
        self.args = args
        self.loss = args.loss
        self.sfloss = args.SFLoss
        self.weight_version = torch.tensor([1,1] + args.weight_version,dtype=torch.float)
        self.alpha = args.alpha

    def forward(self, preds, preds2, target_cls, target_reg, mode):
        """
        :param preds: model predictions in B x C x W x H format (batch size, channels, width, height)
        :param targets: targets in B x W x H format
        :param target_time:
        ...
        stn_preds,stn_preds2,stn_targets_cls, stn_targets_reg = preds,preds2,target_cls.squeeze(1), target_reg.squeeze(1)
        # if self.weight_version == 0:
        #     ce_weight = torch.tensor([1,1,1,1],dtype=torch.float) # 原始版
        # elif self.weight_version == 1:
        #     ce_weight = torch.tensor([1,1,5,5],dtype=torch.float) # 保守版
        # elif self.weight_version == 2:
        #     ce_weight = torch.tensor([1,1,20,20],dtype=torch.float) # 保守加强版
        # elif self.weight_version == 3:
        #     ce_weight = torch.tensor([1,1,2,20],dtype=torch.float) # 比例依赖版
        # elif self.weight_version == 4:
        #     ce_weight = torch.tensor([1,1,4,40],dtype=torch.float) # 比例依赖加强版
        # elif self.weight_version == 5:
        #     ce_weight = torch.tensor([1,1,10,100],dtype=torch.float) # 超级比例依赖加强版
        # elif self.weight_version == 6:
        #     ce_weight = torch.tensor([1,1,50,50],dtype=torch.float) # 倒反天罡版
        # else:
        #     print('!!!!!!!!!!!!Weight Not Implement Yet!!!!!!!!!!!!')

        pred_labels, target_labels = self.get_stat(stn_preds, stn_targets_cls, mode=mode)
        # stn_preds_trans = torch.transpose(stn_preds, 1, 2)
        if self.loss == 'ce':
            loss = nn.CrossEntropyLoss(ignore_index=-1)(stn_preds, stn_targets_cls) 
        elif self.loss == 'mse':
            loss = nn.MSELoss(reduction='mean')(stn_preds2.squeeze(1).float(), stn_targets_reg.float())
        elif self.loss == 'ce+mse':
            # print(stn_preds.shape)
            # print(stn_targets_cls.shape)
            mask = (stn_targets_reg>=self.sfloss)
            mask_m1 = (stn_targets_reg<self.sfloss)
            masked_targets_cls = torch.where(mask_m1, torch.tensor(-1).to(stn_targets_cls.device), stn_targets_cls)
            loss_reg = nn.MSELoss(reduction='sum') \
                (stn_preds2.squeeze(1).float()*mask, stn_targets_reg.float()*mask)/mask.sum()
            loss_cls = nn.CrossEntropyLoss(weight=self.weight_version.to(self.device), ignore_index=-1) \
                (stn_preds, masked_targets_cls)

            loss = loss_cls + self.alpha * loss_reg
            # loss = self.alpha * loss_cls + loss_reg
            # loss = nn.CrossEntropyLoss(weight=ce_weight.to(self.device))(stn_preds, stn_targets_cls) + self.alpha * nn.MSELoss(reduction='mean')(stn_preds2.squeeze(1).float(), stn_targets_reg.float()).mean()
            ...
            loss = FocalLoss(gamma=0.5)(stn_preds, stn_targets_cls)
        elif self.loss == 'focal+mse':
            loss = FocalLoss(gamma=0.5)(stn_preds, stn_targets_cls)+ nn.MSELoss(reduction='none')(stn_preds2.squeeze(1).float(), stn_targets_cls.float()).mean()
        elif self.loss == 'gce':
            loss = GeneralizedCrossEntropy(num_classes=self.num_classes)(stn_preds, stn_targets_cls)
        elif self.loss == 'mae':
            loss = MeanAbsoluteError(num_classes=self.num_classes)(stn_preds, stn_targets_cls)      
        elif self.loss == 'ce+mae':
            loss = CEandMAE(beta = 1.0, num_classes=self.num_classes)(stn_preds, stn_targets_cls)  
        elif self.loss == 'nce+mae':
            loss = NCEandMAE(alpha = 1.0, beta = 1.0, num_classes=self.num_classes)(stn_preds, stn_targets_cls)
        elif self.loss == 'nce+rce':
            loss = NCEandRCE(alpha = 1.0, beta = 1.0, num_classes=self.num_classes)(stn_preds, stn_targets_cls)
        elif self.loss == 'nce':
            loss = NormalizedCrossEntropy(num_classes=self.num_classes)(stn_preds, stn_targets_cls)
        elif self.loss == 'nfl+mae':
            loss = NFLandMAE(alpha = 1.0, beta = 1.0,num_classes=self.num_classes)(stn_preds, stn_targets_cls)        
        elif self.loss == 'nfl+rce':
            loss = NFLandRCE(alpha = 1.0, beta = 1.0,num_classes=self.num_classes)(stn_preds, stn_targets_cls)
        elif self.loss == 'nfl+mse':
            loss = NormalizedFocalLoss( scale=10.0, gamma=0.5, num_classes=self.num_classes)(stn_preds, stn_targets_cls)+ nn.MSELoss(reduction='none')(stn_preds2.squeeze(1).float(), stn_targets_cls.float()).mean()
        elif self.loss == 'nfl':
            loss = NormalizedFocalLoss( scale=10.0, gamma=0.5, num_classes=self.num_classes)(stn_preds, stn_targets_cls)
        elif self.loss == 'ngce+mae':
            loss = NGCEandMAE(alpha = 1.0, beta = 1.0,num_classes=self.num_classes)(stn_preds, stn_targets_cls)
        elif self.loss == 'ngce+rce':
            loss = NGCEandRCE(alpha = 1.0, beta = 1.0,num_classes=self.num_classes)(stn_preds, stn_targets_cls)
        elif self.loss == 'ngce':
            loss = NormalizedGeneralizedCrossEntropy(num_classes=self.num_classes)(stn_preds, stn_targets_cls)
        # elif args.loss == 'nlnl':
            # loss = NCEandMAE(num_classes=self.num_classes)(stn_preds, stn_targets)
        elif self.loss == 'rce':
            loss = ReverseCrossEntropy(num_classes=self.num_classes)(stn_preds, stn_targets_cls)
        elif self.loss == 'sce':
            loss = SCELoss(alpha = 0.1, beta = 1.0,num_classes=self.num_classes)(stn_preds, stn_targets_cls)
        else:
            print('Not Implement Yet！')
    
        return loss, loss_cls, loss_reg, pred_labels, target_labels    
