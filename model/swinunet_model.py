...
class SwinUnet(nn.Module):
    def __init__(self, img_size=224, num_classes=21843, in_chans=3, zero_head=False, vis=False):
...
class SwinUnet_Two(nn.Module):
    def __init__(self, img_size=224, num_classes=21843, in_chans=3, zero_head=False, vis=False):
        ...
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.swin_unet = SwinTransformerSys_Two(img_size=img_size,
                                patch_size=4,
                                in_chans=in_chans,
                                num_classes=num_classes,
                                embed_dim=96,
    ...
    def forward(self, x, target_time=None):
        ...
        # print(f'x.shape at first: {x.shape}')
        if x.size()[1] == 1:
            x = x.repeat(1,3,1,1)
        ...
        logits1 = rearrange(logits1, '(b t) c h w -> b t c h w', c = num_class, t = T).mean(dim=1)
        logits2 = rearrange(logits2, '(b t) c h w -> b t c h w', c = 1, t = T).mean(dim=1)       
        return logits1,logits2

   
class SwinUnet_CAM_Two(nn.Module):
    def __init__(self, img_size=224, in_chans=84, num_classes=21843, zero_head=False, vis=False):
        super(SwinUnet_CAM_Two, self).__init__()
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.ca = ChannelAttention(in_planes=in_chans*4, ratio=16)         
        self.swin_unet = SwinTransformerSys_Two(img_size=img_size,
                                patch_size=4,
                                in_chans=in_chans,
                                num_classes=num_classes,
    ...
    def forward(self, x, target_time=None):
        ...
        # logits1 = F.pad(logits1, (left, right, top, bottom), 'constant', 0)  
        # logits2 = F.pad(logits2, (left, right, top, bottom), 'constant', 0)
        logits1 = rearrange(logits1, '(b t) c h w -> b t c h w', c = 4, t = T).mean(dim=1)
        logits2 = rearrange(logits2, '(b t) c h w -> b t c h w', c = 1, t = T).mean(dim=1)       
        return logits1,logits2
